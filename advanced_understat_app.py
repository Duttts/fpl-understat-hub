...
gw_window = st.sidebar.slider(
    ...
    max_value=10,
    value=5,
    help="Filters ALL tabs (including player tables) to only include data from each team's last N matches.",
)
...
# --- ASYNC FETCHERS ---
async def fetch_match_players(session, understat, match_id):
    """Fetches player stats for a single match."""
    try:
        roster = await understat.get_match_players(match_id)
        match_stats = []
        for team_side in ["h", "a"]:
            if team_side in roster:
                for p_id, p_data in roster[team_side].items():
                    p_data["match_id"] = match_id
                    match_stats.append(p_data)
        return match_stats
    except Exception:
        return []
...
async def fetch_recent_league_data(season_year, recent_matches_count=5):
    async with aiohttp.ClientSession() as session:
        ...
        # 1. Fetch finished match results
        ...
        # 2. Extract recent match IDs per team
        ...
        # 3. Fetch detailed player stats match-by-match for recent matches
        tasks = [
            fetch_match_players(session, understat, m_id)
            for m_id in recent_match_ids
        ]
        match_player_data_nested = await asyncio.gather(*tasks)

        # Flatten list of player match performances
        all_recent_player_matches = [
            item
            for sublist in match_player_data_nested
            if sublist
            for item in sublist
        ]

        return {
            "recentPlayerMatches": all_recent_player_matches,
...
def load_understat_data(season_year=2026, window=5):
    try:
        ...
        if not data or "recentPlayerMatches" not in data:
...
with st.spinner(
    f"Aggregating player metrics for the last {gw_window} matches per team..."
):
...
if not data:
    ...
    # ==========================================
    # TAB 1: PLAYER RECENT METRICS (SUMMED OVER LAST N MATCHES)
    # ==========================================
    with tab1:
        raw_p_matches = pd.DataFrame(data["recentPlayerMatches"])

        if raw_p_matches.empty:
            st.warning("No recent match performance logs available.")
        else:
            numeric_cols = [
                "time",
                "goals",
            ...
            for col in numeric_cols:
                if col in raw_p_matches.columns:
                    raw_p_matches[col] = pd.to_numeric(
                        raw_p_matches[col], errors="coerce"
                    ).fillna(0)

            # Aggregate per player over the recent match window
            df_players = (
                raw_p_matches.groupby(["player_id", "player", "team_title"])
                .agg(
                ...
                .reset_index()
                .rename(
                    columns={"player": "player_name", "player_id": "id"}
                )
            )

            # Calculate Per 90 metrics for the recent window
            df_players["xG_p90"] = (
            ...
            st.subheader(
                f"Player Metrics Leaderboard - Last {gw_window} Matches"
    ...
    with tab2:
        ...
        if "df_players" in locals() and not df_players.empty:
            player_options = (
            ...
            with st.spinner(
                f"Loading recent shots for {selected_player_name}..."
            ):
            ...
            if df_shots.empty:
                ...
                if df_recent_shots.empty:
                    ...
                    c1.metric(
                        f"Shots (Last {gw_window} Games)",
                        f"{total_recent_shots}",
                    )
                    c2.metric(
                        f"xG (Last {gw_window} Games)",
                        f"{total_recent_xg:.2f}",
                    )
                    c3.metric(
                    ...
                    fig = px.scatter(
                        ...
                        title=(
                            f"{selected_player_name} - Shots in Last"
                            f" {gw_window} Matches"
                        ),
                    )
